...
86. **Consent Kind Boundary**: A point consent authorises an action to run NOW with the clinician present and stops minting proofs after a short window; an unexecuted grant does not stay executable forever. A standing consent — the separately-worded "keep doing this for me" — carries a mandatory expiry and is the ONLY thing that authorises a background run to act. A point proof, however fresh, can never authorise unattended work: nobody is there.
87. **Standing Consent Is Non-Disclosing**: Only scopes on an explicit allowlist may ever be held as standing consent, and the allowlist contains no disclosing scope. A new scope is non-standing until someone deliberately adds it — the safe direction for a list whose failure mode is the agent sharing something in the background. The agent may do work for you unattended; it may not disclose you unattended.
88. **Lapse Is Not Withdrawal**: A consent past its window or expiry stays `granted` in the ledger and is marked lapsed. The clinician did approve; the approval is merely unusable now, so the honest surface is "renew", never "you never agreed". Expiry is evaluated as a read-time predicate against an injected clock — nothing sweeps the ledger to mark grants expired, because a background writer racing the head is the ambiguity the seq design exists to prevent.
