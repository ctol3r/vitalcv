...
9. **UX-02 motion values are SET — the EC-20 animation row's delegation is discharged (2026-08-15, founder GO; PR #1410).** The four-band structure holds unchanged; the values inside it are now: control feedback **120ms** (`--duration-instant`/`--duration-snap`/`--duration-respond`/`--vt-motion-control`, per R-e above), state transition **200ms** (`--duration-fast`/`--vt-motion-fast` — the band was previously empty; the 280ms value moved into it had only `tokens.css`'s own zero-consumer presets as references), product transformation **320/380/420ms** (unchanged), rare narrative **600ms** (`--duration-narrative`, new). One easing family, `--vt-ease-system`. `apps/web/styles/tokens.css` is the single source; the TS mirror and the runtime `motionCssVariables` emission derive from it, and a sync suite (with EC-29 band-membership assertions) fails any site that drifts — the mirrors were still shipping the pre-R-e 280ms "instant" four days after #1348 corrected the CSS, which is the defect class the suite exists to catch. Zero-consumer `--vt-duration-interact` removed. The unmounted design-system controls (Button/Input/Tabs/Card) now take control-feedback timing so **D′-02's kit is band-correct before it mounts**. Direction context and the working record: `docs/design/MOTION.md`. Remaining motion debt is real but separately owned: `easy-home.css` hardcodes 44 transitions (several out-of-band — repointing is public-homepage work needing founder visual-gate evidence), and LINT-03's keyframe ratchet holds at 99.
