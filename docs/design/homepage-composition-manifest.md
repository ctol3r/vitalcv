...
**Established:** 2026-07-20 (deep-audit W0.2) · **Rewritten:** 2026-08-08 (UX-V1 production cutover) · **Amended:** 2026-08-13 (Direction D.1 human+tactile career-mobility release; WO-12)
...
exact packet → employer review → accepted head start → reuse. Green describes
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Hero — “One career record. More ways forward.” + real NPI entry | `EasyHome` + `NpiEntry` + `HeroStage` | Static founder copy; live lookup via `useCareerLoop`; `VisualScene` manifest asset | None | SSR-complete form, documentary poster, and tactile folio; a real lookup replaces the illustration | Primary conversion: NPI → live profile → onboarding handoff; secondary: explore opportunities |
...
| 4 | Career-mobility sequence | `CareerMobilitySequence` | Static contract explanation; no result or metric data | CSS transitions only; no loop or engine | Every state and both decision/consent boundaries are server-rendered | Understand the complete accepted-evidence loop in one visual sequence |
