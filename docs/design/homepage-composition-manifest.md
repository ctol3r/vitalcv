...
| 8d | Career constellation | `w1501/Sky` (`SkySection`), scoped in `.w1501` | Fixed illustrative arc — 10 events, **no NPI binding** | Own pointer-drag rotation + a Began→Now→Headed horizon slider | SSR-complete at a deterministic 720px width; static arc, drag disabled, hint hidden under reduced motion | Show what a career record looks like as a shape over time | **MOUNTED (2026-07-22)** — ported from the wave1501 handoff (DG-7.8). **Deliberately NOT in the hero**: row 1's field resolves real lanes from `/api/identity/bootstrap` + `/api/trust-state` once an NPI is entered, and this arc is fixture data, so promoting it to the hero would trade a truthful graph for a prettier fake one in the slot that promises "enter your NPI and see what is real". Below row 8c, labelled illustrative, it restates nothing. The solid/dashed encoding is derived from each event's own fixed `t` against `NOW_T`, never from the slider — the horizon changes emphasis only and can never promote an illustrative event. Raised the `home-npi-role-doors` H2 cap 4 → 5. |
