...
**Established:** 2026-07-20 (deep-audit W0.2) · **Rewritten:** 2026-08-08 (UX-V1 production cutover) · **Amended:** 2026-08-14 (Direction D.7 warm-glass motion synthesis; WO-17)
...
warm paper and ink. A code-authored career horizon supplies movement without a
photo, raster illustration, canvas, or WebGL; the existing server-visible
record is composed as a frosted CV Wallet register inside a luminous warm stage;
...
| Career-horizon display | `WorkSurface` (`data-home-motion-display`) | One-shot CSS path point, aura settle, and glass light catch; the existing record-row assembly shares the same hydrated class | Complete code-authored illustration and all four record rows are server-rendered | Make NPI -> distinct source states -> source-labelled work spatially legible without asserting a real clinician, employer, credential, or result |
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Hero — “One career record. More ways forward.” + real NPI entry | `EasyHome` + `NpiEntry` + `HeroStage` + `WorkSurface` | Static founder copy; live lookup via `useCareerLoop`; code-authored horizon | `WorkSurface` hydration class; one shot only | SSR-complete form, luminous code-authored stage, frosted record, and all truth rows; a real lookup replaces the illustration | Primary conversion: NPI → live profile → onboarding handoff; secondary: explore opportunities |
