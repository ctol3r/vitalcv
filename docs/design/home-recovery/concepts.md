...
**Concept C shipped.** Merged as `a9fd54d73278f5eccf681e59194ad88c4957fa69` on
2026-08-03 and verified live: web and API production both serve that SHA, the
canonical deploy smoke passes 18/18, and the production screenshots match the
approved views. Founder decision record: PR #1062; implementation and pixel
approval: PR #1063.

The two grafts named in the recommendation were both built: Concept B's claim
inspector (as the real `ProofPacketInspector`, not a copy of it) and Concept A's
chapter rail (as anchor-based navigation, so it cannot become a second scroll
owner). The rail's printed `01`–`05` ordinals were removed before merge because
`doctrine-visual` correctly caught them as the numbering CD-13 retires.
