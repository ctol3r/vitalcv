...
| G3 | ~~Rate-limit keying: per-process in-memory counters, `req.ip` fallback with no `trust proxy` configured~~ **Partially closed 2026-07-06:** `app.set('trust proxy', 1)` landed (PR #586), so `req.ip` resolves to the real client behind Railway's edge. Counters remain per-process in-memory (per-instance under scale-out) | `app.ts` (trust proxy set); `publicSafety.ts:55-68` (in-memory keying remains) | Distributed limiter store = follow-up; scored V13 rate-limit row |
