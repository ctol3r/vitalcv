...
| Area | Current % | Gap | Exact Blocker | Fastest Evidence Needed | Proposed Wave/PR | Today? |
|---|---:|---:|---|---|---|---|
| Signup / account creation | 10 | 90 | No real auth provider wired to production; Clerk/NextAuth config not in main; no e2e signup test | Wire Clerk in production env; add e2e signup + email-verification test via Playwright | code: clinician-auth-signup | NO |
...
| LinkedIn-style profile layer | 28 | 72 | `verifiesCredentials:false`; LinkedIn OAuth not wired; scrape/import not live | LinkedIn OAuth import route (or scrape alternative); import→profile field binding + test | code: linkedin-import-live | NO |
| Doximity-style profile layer | 26 | 74 | Same as LinkedIn; Doximity OAuth not wired; `verifiesCredentials:false` | Doximity OAuth or scrape import route; profile field binding + test | code: doximity-import-live | NO |
...
| Real persistence writer | 5 | 95 | No Prisma table; no audit-event table; no client-safe RPC; SQLite/in-memory only | Add Prisma schema with core tables; run migration; implement create/read for issuer receipt; test | code: prisma-core-schema | NO |
...
| Database migration readiness | 5 | 95 | SQLite + in-memory only; no PostgreSQL schema; no migration tooling configured | Provision PostgreSQL in Railway; write Prisma schema + initial migration; smoke test in CI | code: db-migration-postgres | NO |
