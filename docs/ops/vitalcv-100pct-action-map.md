...
| Database migration readiness | 5 | 95 | SQLite + in-memory only; no PostgreSQL schema; no migration tooling configured | (1) Write dry-run SQL doc under `docs/migrations/` for review; (2) get explicit approval; (3) edit `apps/api/backend/prisma/schema.prisma` with the approved changes; (4) `cd apps/api/backend && npx prisma migrate dev --schema prisma/schema.prisma --name <description>` to generate the migration under `apps/api/backend/prisma/migrations/`; (5) provision PostgreSQL in Railway; (6) apply via `cd apps/api/backend && npx prisma migrate deploy --schema prisma/schema.prisma`; smoke test in CI | code: db-migration-postgres | NO |
