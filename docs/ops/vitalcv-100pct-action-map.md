...
| Area | Current % | Gap | Exact Blocker | Fastest Evidence Needed | Proposed Wave/PR | Today? |
|---|---:|---:|---|---|---|---|
| Signup / account creation | 10 | 90 | Clerk/NextAuth already wired in main (`@clerk/nextjs` in package.json, `ClerkProvider` in providers.tsx, `/sign-up` route); gap is production env keys + e2e verification, not auth setup | Add e2e signup + email-verification test via Playwright; confirm Clerk production keys in env; smoke-test session persistence | code: clinician-auth-signup | NO |
...
| Real persistence writer | 5 | 95 | No Prisma table; no audit-event table; no client-safe RPC; SQLite/in-memory only | Write approval-gated migration plan doc under docs/architecture; get explicit approval before any migration execution; then implement Prisma create/read for issuer receipt with tests | code: prisma-core-schema | NO |
...
| Database migration readiness | 5 | 95 | SQLite + in-memory only; no PostgreSQL schema; no migration tooling configured | Write PostgreSQL migration plan doc under docs/architecture first; get explicit approval; then provision PostgreSQL in Railway; write Prisma schema + run approval-gated migration; smoke test in CI | code: db-migration-postgres | NO |
