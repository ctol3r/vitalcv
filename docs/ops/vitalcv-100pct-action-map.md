...
| Database migration readiness | 5 | 95 | SQLite + in-memory only; no PostgreSQL schema; no migration tooling configured | Write dry-run PostgreSQL migration SQL under `docs/migrations/` first; get explicit approval; then provision PostgreSQL in Railway; apply migration via `prisma migrate deploy`; smoke test in CI | code: db-migration-postgres | NO |
