...
- **Tightens** actor-scoped attribution, replay resistance, audit coupling, readonly enforcement, and fail-closed mutation semantics.
...
| `if (!correlationId) { correlationId = randomUUID() }` AT THE BACKEND | Idempotency anchor must come from the actor / proxy; backend-generated correlation IDs defeat replay resistance. (Proxy may generate one if missing; the proxy then forwards it.) |
...
| 7.4 — Replay resistance via correlationId (5 actions × 1 scenario) | 5 | Each action with duplicate `(actorId, correlationId, 24h)` returns 409 `duplicate_request` and writes NO new audit row |
...
**Total: 28 cases.** Down from v1's 34 — the 6 cross-tenant cases (per §7.1 & §7.3 of v1) are removed because cross-tenant comparison is deferred. The 28-case suite locks the legitimacy contract: readonly cannot mutate, mutations are atomic with audit, replays do not duplicate, forbidden inputs are discarded, header injection is defeated, edge cases fail closed.
...
## 8. Mandatory audit writes (atomic with mutation)
...
| Verified failure of replay resistance (duplicate persisted state on retry) | Idempotency key not enforced; merge was incorrect |
