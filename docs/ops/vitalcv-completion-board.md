...
| Analytics | 35 | 90 | 55 | +0 | +20 | (FOUNDATION-SWEEP-6B) `apps/web/lib/commercial/analyticsFoundation.ts` defines a 6-event privacy-safe vocabulary (npi_lookup_started / profile_section_viewed / import_entry_clicked / proof_pack_previewed / signup_step_viewed / verifier_cta_clicked) with typed `dispatchedToThirdParty: false`, `productionPipelineLive: false`, `collectsPhi: false`, `collectsCredentialPayload: false` invariants; allowedContextKeys exclude NPI/email/name/credential by construction; surfaced on `/analytics-foundation` route; no PostHog/Segment/Mixpanel/GA4 vendor wired | 🧱 Foundation |
