...
| Analytics | 40 | 90 | 50 | +0 | +20 | (FOUNDATION-SWEEP-6B) `apps/web/lib/commercial/analyticsFoundation.ts` defines a 6-event privacy-safe vocabulary (npi_lookup_started / profile_section_viewed / import_entry_clicked / proof_pack_previewed / signup_step_viewed / verifier_cta_clicked) with typed `dispatchedToThirdParty: false`, `productionPipelineLive: false`, `collectsPhi: false`, `collectsCredentialPayload: false` invariants; every event privacy level is one of `safe_session_only` / `safe_aggregate` (no `requires_consent` in catalog); allowedContextKeys whitelist excludes NPI/email/name/phone/address/SSN/DOB/credential/token/secret by construction; surfaced on `/analytics-foundation` route with verbatim "No PHI or credential payloads are collected" disclaimer; no PostHog/Segment/Mixpanel/GA4 vendor wired; **production pipeline not live** | 🧱 Foundation |
| Docs / status page | 45 | 90 | 45 | +0 | +20 | (FOUNDATION-SWEEP-6B) `apps/web/lib/commercial/statusFoundation.ts` defines 6-surface plan (source_health_status / build_status / docs_index / public_changelog_planned / incident_notice_planned / api_docs_planned) with typed `uptimeGuaranteeImplied: false`, `productionStatusPageLive: false`, `apiDocsComplete: false` invariants; surfaced on `/status` and `/docs` routes; legal pages (`/privacy`, `/terms`) live (#LIVE-100C) | 🧱 Foundation |
