...
* Truth audit: CTA reworded from "identity verified in 1.8 min" → "readiness to employer action in 1.8 min" to match pilot evidence on `/p/norcal-pa-pilot-1`. NPDB removed from T3 tier example entirely (HomePageClient.tsx:76) — T3 example now shows only integrated sources (NPPES · OIG/LEIE · PECOS); NPDB not mentioned per source-coverage-matrix.
