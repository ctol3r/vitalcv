...
| Live behavior validation | **Still pending.** Authenticated SSE smoke for NPI 1699264564 not yet run (requires operator-signed-in browser session — `POST /api/ingest/1699264564` returns 403/x-cors-blocked unauthenticated). Until smoke confirms NPPES `source_complete` `status:"SUCCESS" + resultStatus:"SUCCESS"` for intact payload (or correctly stays `FAILED` for no-payload), behavior is "deployed" not "validated live". |
