...
| Secret | Purpose |
| --- | --- |
| `CLERK_SECRET_KEY_PROD` | Clerk Backend API — mint/cleanup the synthetic clinician. Mapped onto the `CLERK_SECRET_KEY` env var in-workflow. Rotating it: [`clerk-secret-key-rotation.md`](clerk-secret-key-rotation.md) — read it first, the key also signs role cookies. |
