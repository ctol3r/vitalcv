...
| **`/directory/[npi]` latency** | Cold render **8.22–8.30s** across five seed NPIs; ISR-warm 0.15–0.53s. A crawler sweeping the 4,955 seeded NPIs pays cold on essentially every request. | **Not** a synchronous-NPPES problem, as first guessed — NPPES measures 0.20s and the exact CMS query 0.26–0.47s, and the 8s `TIMEOUT_MS` match is a coincidence (the CMS block renders real data). Needs local profiling or Railway logs. |
...
| **Orphaned decision-console pair** (corrected: not a live defect) | `ConsoleWrapper.tsx` POSTs to a nonexistent route but is itself imported by nothing; the live `/review/[entityId]` surface acts through the real authenticated endpoints. The hazard: the unmounted backend `routes/employer-action.ts` is **unauthenticated** (body-supplied `employerId`, fabricated audit hashes) — one accidental mount away from an open acceptance write. Predates this cycle (#140/#148, 2026-04-17). | Delete `ConsoleWrapper.tsx`, `EmployerDecisionConsole.tsx`, and `routes/employer-action.ts`. Wave C1 candidate. |
