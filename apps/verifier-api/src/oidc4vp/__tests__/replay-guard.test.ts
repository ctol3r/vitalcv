...
describe('B125A-OIDC-005: OIDC4VP Replay Guard', () => {
  ...
  describe('JTI Replay Protection', () => {
    ...
    it('should return 409 when VP token with same JTI is reused (replay attack)', async () => {
      ...
      const vpToken2 = await createVPToken(jti);
    ...
    it('should track multiple unique JTIs independently', async () => {
      ...
      const vpToken1 = await createVPToken(jti1);
      const vpToken2 = await createVPToken(jti2);
      const vpToken3 = await createVPToken(jti3);
      ...
      const vpToken1Replay = await createVPToken(jti1);
      const replayResponse = await request(app)
        .post('/oidc4vp/presentation')
        .send({ vp_token: vpToken1Replay });
  ...
  describe('Nonce Binding', () => {
    ...
    it('should allow VP token without nonce (nonce is optional)', async () => {
      const vpToken = await createVPToken();
      const response = await request(app)
        .post('/oidc4vp/presentation')
        .send({
      ...
      // Should not fail due to missing nonce (nonce is optional for VP)
      expect(response.status).not.toBe(400);
      expect(response.body.error_description).not.toContain('nonce');
    });
  });
  ...
  describe('E2E Positive Flow', () => {
    it('should support complete VP presentation flow with nonce', async () => {
      ...
      expect(presentationResponse.body.verified).toBe(true);
    ...
    it('should support multiple presentations with different JTIs and nonces', async () => {
      ...
      const vpToken1 = await createVPToken();
      const presentation1 = await request(app)
        .post('/oidc4vp/presentation')
        .send({
          vp_token: vpToken1,
      ...
      const vpToken2 = await createVPToken();
      const presentation2 = await request(app)
        .post('/oidc4vp/presentation')
        .send({
          vp_token: vpToken2,
      ...
      expect(presentation1.body.verified).toBe(true);
      expect(presentation2.body.verified).toBe(true);
    });
  });
});
