...
// B119A-EUDI-009: DPoP guard - local middleware (shared logic)
import { dpopGuard } from '../middleware/dpopGuard';
import { featureFlagGuard } from '../services/featureFlags';
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!trustList && trust_list_url && public_key) {
      ...
      trustList = fetchResult.trustList ?? null;
    }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const claimsRequested: string[] = Array.isArray(claims_requested)
        ? claims_requested.map((claim) => String(claim))
        : typeof claims_requested === 'string'
        ? [claims_requested]
        : [];
