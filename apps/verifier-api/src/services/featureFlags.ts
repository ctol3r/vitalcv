...
const log = createConsoleLogger('flags/featureFlags');

type PrismaClient = any;
type PrismaClientCtor = new () => PrismaClient;

let prismaClientCtor: PrismaClientCtor | null | undefined;

function resolvePrismaClientCtor(): PrismaClientCtor | null {
  if (prismaClientCtor !== undefined) {
    return prismaClientCtor;
  }

  try {
    const prismaModule = require('@prisma/client') as { PrismaClient?: PrismaClientCtor };
    prismaClientCtor = prismaModule.PrismaClient ?? null;
  } catch {
...
function createPrismaClient(): PrismaClient | null {
  const ctor = resolvePrismaClientCtor();
 ...
 */
export function getFeatureFlagService(prisma?: PrismaClient | null): FeatureFlagService {
  if (!featureFlagInstance || (prisma && !featureFlagInstance.hasPrisma())) {
    featureFlagInstance = new FeatureFlagService(prisma ?? null);
  }
...
export function featureFlagGuard(
  ...
  return async (req: any, res: any, next: any) => {
    try {
      const prismaFromReq = (req as any).prisma as PrismaClient | undefined;
      const prisma = prismaFromReq ?? createPrismaClient();
