...
export async function runCommitteePacketNightly(referenceDate = new Date()): Promise<CommitteePacketNightlyStats> {
  ...
  try {
    const clinicians = await prisma.clinicianProfile.findMany({
      select: { id: true, user: { select: { did: true } } },
    });
    ...
    for (const clinician of clinicians) {
      ...
      try {
        ...
        const newestSourceUpdate = await getLatestSourceUpdate(clinician.id, clinician.user?.did ?? null);
...
async function getLatestSourceUpdate(clinicianId: string, clinicianDid: string | null): Promise<Date | null> {
  ...
  const timestamps = [license?.updatedAt, payer?.updatedAt, privilege?.updatedAt, quality?.updatedAt].filter(Boolean) as Date[];
