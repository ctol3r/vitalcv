...
import { runPSV, PSVOrchestratorInput } from '../../../services/psv/psvOrchestrator.js';
import { recordPilotSmokeRun } from '../../../../services/pilot/pilotAnalytics.js';
 ...
 */
async function issueClinicianIdentityVC(): Promise<any> {
 ...
 */
async function runPSVStub(vcResult: any): Promise<any> {
  console.log(`🔍 [Smoke Test] Running PSV for NPI: ${vcResult.npi}`);

  const psvInput: PSVOrchestratorInput = {
  ...
  const psvResult = await runPSV(psvInput);
 ...
 */
async function createPrivilegeRequest(vcResult: any, psvResult: any): Promise<any> {
 ...
 */
async function approvePrivilegeRequest(privilegeRequestResult: any): Promise<any> {
 ...
 */
export async function runPilotSmoke(): Promise<PilotSmokeResult> {
  const startTime = Date.now();
  ...
  try {
    ...
    const vcStep = await executeStep(
      'issue_clinician_identity_vc',
    ...
    // Step 2: Run PSV
    const psvStep = await executeStep(
      'run_psv_stub',
      () => runPSVStub(vcStep.details)
    );
    steps.push(psvStep);
    ...
    // Step 3: Create PrivilegeRequest
    const privilegeRequestStep = await executeStep(
      'create_privilege_request',
      () => createPrivilegeRequest(vcStep.details, psvStep.details)
    );
    ...
    const approvalStep = await executeStep(
      'approve_privilege_request',
      () => approvePrivilegeRequest(privilegeRequestStep.details)
    );
