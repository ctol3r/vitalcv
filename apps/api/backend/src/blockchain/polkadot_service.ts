...
export interface ChainClientConfig {
  ...
  /** Max connection attempts before failing. */
  maxConnectAttempts: number;
  /** Base backoff (ms) between retries. */
  retryBackoffMs: number;
  /** Consider the node "lagging" if best - finalized exceeds this. */
  maxAllowedLagBlocks: number;
}
...
export class PolkadotService {
  ...
  constructor(initialKey?: string, cfg?: Partial<ChainClientConfig>) {
    ...
    this.cfg = {
      ...
      connectTimeoutMs: cfg?.connectTimeoutMs ?? 12_000,
  ...
  async connect(endpoint?: string | string[]): Promise<void> {
    ...
    for (let attempt = 0; attempt < this.cfg.maxConnectAttempts; attempt++) {
      ...
      try {
        const provider = new WsProvider(ep, this.cfg.connectTimeoutMs);
