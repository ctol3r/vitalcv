...
export function registerIntelligenceEngineRoutes(app: Express): void {
  ...
  app.get('/api/intelligence/status', (_req: Request, res: Response) => {
    res.json({ schema: 'https://vitalcv.com/intelligence/v1', ...getOrchestratorStatus() });
  });
  ...
  app.post('/api/intelligence/cycle', async (req: Request, res: Response) => {
    ...
    try {
      const result = await runOrchestrationCycle({
        npis: body?.npis, maxIngestions: body?.maxIngestions, skipInsights: body?.skipInsights,
      });
  ...
  app.get('/api/intelligence/insights', async (req: Request, res: Response) => {
    ...
    try {
      const insights = fresh ? await generateInsights({ limit: limitParam }) : getCachedInsights();
      res.json({ count: insights.length, insights, cached: !fresh, generatedAt: new Date().toISOString() });
    } catch (err) {
  ...
  app.get('/api/intelligence/learning', (_req: Request, res: Response) => {
    res.json({ schema: 'https://vitalcv.com/decision-learning/v1', ...getDecisionLearningState() });
  });
  ...
  app.post('/api/intelligence/learning/record', (req: Request, res: Response) => {
    ...
    if (!suggestionId || (action !== 'accept' && action !== 'reject')) {
      return res.status(400).json({ error: 'Body must include suggestionId and action (accept|reject)' });
    }
