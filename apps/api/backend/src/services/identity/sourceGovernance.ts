...
export const SOURCE_GOVERNANCE: Record<string, SourceGovernance> = {
  ...
  PECOS_PUBLIC: {
    ...
    recommendedRefresh: 'Monthly; enrollment status rarely changes faster',
    automationNotes: 'data.cms.gov Socrata API. Free, no key needed.',
    openQuestions: [],
    buildStatus: 'live',
  ...
  OIG_LEIE: {
    ...
    automationNotes: 'Monthly CSV bulk + real-time search. API error = UNKNOWN status, cap at L1. Never silently pass on failure.',
    openQuestions: [],
    buildStatus: 'live',
    implementationPriority: 1,
  },
