...
function buildMerkleTreeFromLeafHashes(leafHashes: readonly string[]): MerkleTree {
...
export function buildMerkleTree(claims: readonly CanonicalClaim[]): MerkleTree {
  ...
  const leafHashes = normalizedClaims.map((claim) => hashClaim(claim));
  return buildMerkleTreeFromLeafHashes(leafHashes);
}

export function generateMerkleProofPath(
  ...
  const leafNodes = normalizedClaims.map((claim) => ({ ...claim, leafHash: hashClaim(claim) }));
