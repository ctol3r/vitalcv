...
function buildReadme(packet: EmployerEvidencePacketV1): string {
  ...
  return [
    ...
    '- Only checked sources are decision-grade primary-source proof.',
    '- Stale, gated, access-required, review-required, unavailable, preview-only, and not-decision-grade sources stay contextual until revalidated or completed.',
    '',
    'Launch Spine Coverage',
    `- Checked: ${summary.checked.join(', ') || 'none'}`,
    `- Stale: ${summary.stale.join(', ') || 'none'}`,
    `- Pending: ${summary.pending.join(', ') || 'none'}`,
    `- Gated: ${summary.gated.join(', ') || 'none'}`,
    ...
    `- Not decision-grade: ${summary.notDecisionGrade.join(', ') || 'none'}`,
    `- Preview only: ${summary.previewOnly.join(', ') || 'none'}`,
    '',
