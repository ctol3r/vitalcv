...
class PolkadotService {
        ...
        for (let attempt = 0; attempt < this.cfg.maxConnectAttempts; attempt++) {
            ...
            try {
                ...
                await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('health timeout')), this.cfg.connectTimeoutMs)),
                ]);
                this.api = api;
    ...
    async batchIssueCredentials(signer, destinations, data) {
        ...
        return this.withApi(async (api) => {
            const calls = destinations.map((dest, i) => api.tx.credentialsModule.issueCredential(dest, data[i]));
