...
describe('JWKS public key contract', () => {
  it('getPublicJWKS returns only public EC key fields — no private d parameter', async () => {
    // Import directly from the backend service to test the key-construction logic
    // independently of the HTTP transport layer.
    const { getPublicJWKS } = await import('../../api/backend/src/services/trust/cryptoService');
