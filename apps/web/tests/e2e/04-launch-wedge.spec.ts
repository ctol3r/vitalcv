...
const REVIEW_BLOCKED_COPY = 'No decision card is rendered until VitalCV has both a valid organization context and a hydrated passport record.';
...
async function runLaunchWedgeFlow(page: Page) {
  ...
  await expect(page.getByText(/checking primary sources|preparing demo preview|resolving readiness/i)).toBeVisible();
  await expect(
    page.locator('p').filter({ hasText: /^Readiness snapshot$/ }).first(),
  ).toBeVisible({ timeout: 10_000 });
  await expect(
    page.locator('span').filter({ hasText: /^Demo preview$/ }).first(),
  ).toBeVisible();

  await page.goto('/interview', { waitUntil: 'networkidle' });
  await expect(page.getByText('Interview Mode shows your real verified readiness to an employer.')).toBeVisible();
