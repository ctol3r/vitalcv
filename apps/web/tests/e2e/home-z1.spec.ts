/**
 * Homepage contract — the Z1 product story (founder-directed live 2026-08-03,
 * superseding the six-scene film).
 *
 * This consolidates the durable truths of the retired film specs
 * (npi-truth-engine, ask-home, ask-npi-response, home-phase, home-atmosphere,
 * glass-chrome) against the composition visitors actually reach:
 *
...
test.describe('homepage — the Z1 product story', () => {
  test('deploy contract: marker, one h1, indexable', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('[data-home-hero]')).toBeVisible();
    await expect(page.locator('h1')).toHaveCount(1);
    // '/' must be indexable; the noindex alias is /preview/home.
