...
export interface IntelligenceProvider extends ProviderView {}
...
export interface IntelligenceActionWorkflowEffects {
...
export interface IntelligenceRecommendationDecision {
  actionType: 'ACCEPT_RECOMMENDED';
  label: string;
  confidence: number;
}

export interface IntelligenceRecommendationPreview {
  actionType: string;
  label: string;
  confidence: number;
  explanation: string;
  autoGenerated: boolean;
  createdAt: string;
  workflowEffects: IntelligenceActionWorkflowEffects;
  previewDecision: IntelligenceRecommendationDecision | null;
}
...
export interface ComputeProviderViewInput {
  id?: string | null;
  npi: string;
  name?: string | null;
  fullName?: string | null;
  specialties?: string[] | null;
  credentialHealth?: IntelligenceProvider['credentialHealth'] | string | null;
  trustScore?: number | null;
  readinessScore?: number | null;
  activeCredentials?: number | null;
  credentialCount?: number | null;
  totalCredentialCount?: number | null;
  primaryIssuer?: string | null;
  lastVerifiedAt?: string | null;
  summary?: string | null;
  tags?: string[] | null;
...
function normalizeCredentialHealth(
  value: ComputeProviderViewInput['credentialHealth'],
): IntelligenceProvider['credentialHealth'] {
  ...
  switch (normalized) {
    ...
    case 'REVOKED':
    case 'PENDING':
      return normalized as IntelligenceProvider['credentialHealth'];
    default:
      return 'PENDING';
  }
}
...
function buildProviderSummary(input: {
  ...
  return [
    input.specialties.slice(0, 2).join(', '),
    input.primaryIssuer,
    input.lastVerifiedAt ? `verified ${new Date(input.lastVerifiedAt).toLocaleDateString('en-US')}` : null,
  ]
    .filter((value): value is string => Boolean(value))
    .join(' • ');
}

export function computeProviderView(input: ComputeProviderViewInput): ProviderView {
  const credentialHealth = normalizeCredentialHealth(input.credentialHealth);
  const trustScore = clampScore(input.trustScore)
    ?? clampScore(input.readinessScore)
    ?? 0;
  const readinessScore = clampScore(input.readinessScore)
    ?? trustScore;
  const credentialCount = Math.max(
    0,
    input.totalCredentialCount
      ?? input.credentialCount
      ?? input.activeCredentials
      ?? 0,
  );
  const activeCredentials = Math.max(
    0,
    Math.min(input.activeCredentials ?? credentialCount, credentialCount),
  );
  const specialties = uniqueStrings((input.specialties ?? []).filter((value): value is string => Boolean(value?.trim())));
  const primaryIssuer = asString(input.primaryIssuer);
  const lastVerifiedAt = asString(input.lastVerifiedAt);
  const summary = asString(input.summary) ?? buildProviderSummary({
    specialties,
    primaryIssuer,
    lastVerifiedAt,
  });
  const tags = uniqueStrings([
    ...
    ...((input.tags ?? []).filter((value): value is string => Boolean(value?.trim()))),
  ]);
  const riskLevel = providerRiskFromScore(trustScore, credentialHealth);

  return {
    id: input.id ?? input.npi,
    npi: input.npi,
    name: asString(input.name) ?? asString(input.fullName) ?? `Provider ${input.npi}`,
    specialties,
    credentialHealth,
    trustScore,
    readinessScore,
...
export function normalizeProvidersPayload(
  ...
  const providers = (payload.entries ?? [])
    .map((entry): IntelligenceProvider => computeProviderView({
      id: entry.npi,
      npi: entry.npi,
      ...
      readinessScore: entry.readinessScore ?? entry.trustScore,
...
function normalizeEvidenceList(
  ...
  return (items ?? []).map((item, index) => ({
    ...
    url: item.url ?? null,
    fields: Object.entries(item.metadata ?? {})
      .slice(0, 4)
      .map(([label, value]) => ({
        label,
  ...
  const storylines = (payload.storylines ?? [])
    .map((storyline): IntelligenceStoryline => ({
      ...
      evidence: (storyline.supportingEvidence ?? []).map((evidence, index) => ({
        ...
        relevance: null,
        fields: [
          ...(evidence.findingId ? [{ label: 'findingId', value: evidence.findingId }] : []),
          ...(evidence.artifactId ? [{ label: 'artifactId', value: evidence.artifactId }] : []),
        ],
      })),
...
export function normalizeActionsPayload(payload: ActionListPayload): ActionsResponse {
  const actions = (payload.actions ?? [])
    .map((action): IntelligenceAction => ({
      ...
      recommendationLabel: action.recommendationLabel ?? null,
      recommendationConfidence: typeof action.recommendationConfidence === 'number'
        ? action.recommendationConfidence
        : null,
      workflowEffects: action.workflowEffects ?? null,
      autoGenerated: action.autoGenerated === true,
      previewDecision: action.previewDecision ?? null,
...
type GeneratedActionTemplate = {
  actionType: string;
  priority: string;
  priorityScore: number;
  queueDestination: IntelligenceActionWorkflowEffects['queueDestination'];
  title: (finding: IntelligenceFinding, provider: ProviderView | null) => string;
  explanation: (finding: IntelligenceFinding, provider: ProviderView | null) => string;
};

const GENERATED_ACTION_TEMPLATES: Record<string, GeneratedActionTemplate> = {
  TRUST_DECLINE: {
    actionType: 'REVIEW_REQUIRED',
    priority: 'high',
    priorityScore: 92,
    queueDestination: 'risk_review',
    title: (finding, provider) => `Review trust decline for ${provider?.name ?? finding.providerLabel ?? finding.providerNpi ?? 'provider'}`,
    explanation: (finding) => finding.explanation || finding.summary,
  },
  DIVERGENCE: {
    ...
    title: (finding, provider) => `Verify source divergence for ${provider?.name ?? finding.providerLabel ?? finding.providerNpi ?? 'provider'}`,
    explanation: (finding) => finding.explanation || finding.summary,
  },
  WORKFORCE_SHIFT: {
    ...
    priority: 'medium',
    priorityScore: 81,
    queueDestination: 'review_queue',
    title: (finding, provider) => `Investigate workforce shift for ${provider?.name ?? finding.providerLabel ?? finding.providerNpi ?? 'provider'}`,
    explanation: (finding) => finding.explanation || finding.summary,
  },
};

function normalizedActionKey(action: Pick<IntelligenceAction, 'actionType' | 'providerNpi' | 'sourceFindingIds'>): string {
  return [
...
export function generateActionsFromFindings(
  findings: IntelligenceFinding[],
  providerViews: Iterable<ProviderView> = [],
): IntelligenceAction[] {
  const providersByNpi = new Map<string, ProviderView>();
  ...
  for (const finding of findings) {
    const template = GENERATED_ACTION_TEMPLATES[finding.findingType.trim().toUpperCase()];
    if (!template) {
      continue;
    }

    const provider = finding.providerNpi ? providersByNpi.get(finding.providerNpi) ?? null : null;
    const priorityScore = Math.max(
      template.priorityScore,
      Math.round(finding.priorityScore),
      Math.round(finding.confidence * 100),
    );
    const confidence = Math.max(
      0.5,
      Math.min(0.99, Number.parseFloat(finding.confidence.toFixed(2))),
    );

    actions.push({
      id: `generated:${template.actionType}:${finding.id}`,
      actionType: template.actionType,
      priority: template.priority,
      priorityScore,
      status: 'pending',
      title: template.title(finding, provider),
      explanation: template.explanation(finding, provider),
      confidence,
      recommendationLabel: template.title(finding, provider),
      recommendationConfidence: confidence,
      workflowEffects: {
        queueDestination: template.queueDestination,
        missingCredentials: [],
        employerNotification: false,
        clinicianRequest: template.queueDestination === 'credential_request',
      ...
      previewDecision: {
        actionType: 'ACCEPT_RECOMMENDED',
        label: template.title(finding, provider),
        confidence,
      },
      providerNpi: finding.providerNpi,
      targetLabel: provider?.name ?? finding.providerLabel ?? null,
      sourceFindingIds: [finding.id],
      evidence: finding.evidence.slice(0, 4),
      createdAt: finding.updatedAt ?? finding.firstSeenAt,
    });
  }

  const deduped = new Map<string, IntelligenceAction>();
  for (const action of actions) {
    deduped.set(normalizedActionKey(action), action);
  }
...
export function normalizeCanonicalSystemHealthPayload(
  ...
  return {
    ...
    headline: payload.status === 'HEALTHY'
      ...
      : findings > 0 || storylines > 0
        ? `${findings} findings across ${storylines} storylines are live.`
        : `${providers} providers are indexed and live intelligence is available.`,
...
export function candidateProviderNpisFromGraphNode(node: GraphNode): string[] {
