...
export function CommandCenterDashboard() {
  return (
    <div className="min-h-screen bg-[#0D1117] text-slate-300 font-sans p-6 overflow-hidden flex flex-col gap-6">
      ...
      <header className="flex items-center justify-between pb-4 border-b border-white/10">
        <div>
          ...
          <p className="text-sm text-slate-500 font-mono mt-1">SYSTEM_STATUS: NOMINAL | INGESTION_RATE: 4.2k events/s</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-md text-slate-400 text-sm font-mono">
      ...
      <div className="grid grid-cols-12 gap-6 flex-1 min-h-0">
        ...
        <div className="col-span-3 flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold tracking-widest text-slate-400 uppercase">Active Storylines</h2>
            <span className="text-xs bg-white/10 px-2 py-0.5 rounded-full">12 Live</span>
          </div>
          ...
          {[
            ...
            <motion.div 
              ...
              className={cn(
                "p-4 rounded-xl border-l-[3px] bg-white/5 border-y border-r border-white/10 backdrop-blur-md cursor-pointer transition-colors hover:bg-white/10",
                story.severity === 'high' ? 'border-l-vt-destructive' : 'border-l-vt-warning'
              )}
            >
              <div className="text-xs font-mono text-slate-500 mb-1">{story.id}</div>
              <h3 className="font-semibold text-white text-sm mb-1">{story.title}</h3>
        ...
        <div className="col-span-6 flex flex-col gap-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Card className="bg-gradient-to-br from-vt-destructive/20 to-transparent border-vt-destructive/30">
              ...
              <CardContent>
                <div className="text-4xl font-mono text-white mb-2">3</div>
                <p className="text-sm text-slate-400">Providers flagged for peer-review discrepancies in the last hour.</p>
              </CardContent>
            </Card>
             ...
             <Card className="bg-gradient-to-br from-vt-info/20 to-transparent border-vt-info/30">
              ...
              <CardContent>
                <div className="text-4xl font-mono text-white mb-2">1,402</div>
                <p className="text-sm text-slate-400">New edges formed regarding clinical trial collaborations.</p>
              </CardContent>
            </Card>
          </div>

          <div className="flex-1 bg-white/5 border border-white/10 rounded-xl p-6 backdrop-blur-sm flex flex-col">
            <h2 className="text-sm font-bold tracking-widest text-slate-400 uppercase mb-4">Recommended Actions</h2>
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-black/40 border border-white/5 hover:border-white/20 transition-all group">
                  <div>
                    <div className="text-sm font-medium text-white group-hover:text-vt-info transition-colors">{rec.action}</div>
                    <div className="text-xs text-slate-500 mt-0.5">{rec.context}</div>
                  </div>
                  <button className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white rounded-md text-xs font-semibold uppercase tracking-wide transition-colors">
                    Execute
                  </button>
                </div>
              ))}
        ...
        <div className="col-span-3 flex flex-col gap-6">
          <Card className="bg-[#0a0a0f] border-white/10">
            ...
            <CardContent className="font-mono text-xs space-y-3">
              ...
              <div className="flex justify-between">
                <span className="text-slate-500">Graph Compute</span>
                <span className="text-vt-success">42ms</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Resolver Workers</span>
                <span className="text-white">8/12 ACTIVE</span>
              </div>
          ...
          <div className="flex-1 bg-[#0a0a0f] border border-white/10 rounded-xl p-4 flex flex-col">
            ...
            <div className="space-y-2 flex-1 overflow-y-auto">
              {[
                ...
                <div key={inv.id} className="text-sm flex items-center gap-3 p-2 rounded hover:bg-white/5 cursor-pointer text-slate-300">
