...
export function ActionDetailClient({
  ...
  const providerNpi = /^\d{10}$/.test(detail.action.targetEntity.entityId)
    ...
    : null;
  const workflowEffects = detail.action.workflowEffects ?? null;
  const recommendationLabel = detail.action.recommendationLabel ?? detail.action.recommendedAction;
  const recommendationConfidence = typeof detail.action.recommendationConfidence === 'number'
    ? detail.action.recommendationConfidence
    : detail.action.confidence;
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]">
        <div className="space-y-4">
          <OpsCard className="space-y-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {detail.action.autoGenerated === true ? <OpsBadge label="AUTO GENERATED" tone="info" /> : null}
            </div>
          ...
          <OpsCard className="space-y-4">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-lg font-semibold text-[var(--vt-text-1)]">Recommendation</h2>
              <span className="text-sm text-[var(--vt-text-3)]">{Math.round(recommendationConfidence * 100)}% confidence</span>
            </div>
            <p className="text-base font-semibold text-[var(--vt-text-1)]">{recommendationLabel}</p>
            {detail.action.previewDecision ? (
              <p className="text-sm text-[var(--vt-text-2)]">
                Preview: {detail.action.previewDecision.label} ({Math.round(detail.action.previewDecision.confidence * 100)}% confidence)
              </p>
            ) : null}
            {workflowEffects ? (
              <div className="space-y-3">
                <div className="flex flex-wrap gap-2">
                  <OpsBadge label={workflowEffects.queueDestination.replace(/_/g, ' ')} />
                  {workflowEffects.employerNotification ? <OpsBadge label="EMPLOYER NOTIFY" tone="info" /> : null}
                  {workflowEffects.clinicianRequest ? <OpsBadge label="CLINICIAN REQUEST" tone="warning" /> : null}
                  {workflowEffects.webhookQueued ? <OpsBadge label="WEBHOOK QUEUED" tone="warning" /> : null}
                </div>
                {workflowEffects.missingCredentials.length > 0 ? (
                  <p className="text-sm text-[var(--vt-text-2)]">
                    Missing credentials: {workflowEffects.missingCredentials.join(', ')}
                  </p>
                ) : (
                  <p className="text-sm text-[var(--vt-text-3)]">No blocking credential gaps are attached to this recommendation.</p>
                )}
              </div>
            ) : (
              <p className="text-sm text-[var(--vt-text-3)]">This action does not include hiring workflow metadata.</p>
            )}
          </OpsCard>
        ...
        <div className="space-y-4">
          ...
          <OpsCard className="space-y-3">
            ...
            {typeof detail.action.recommendationConfidence === 'number' ? (
              <p className="text-sm text-[var(--vt-text-2)]">Recommendation confidence {Math.round(detail.action.recommendationConfidence * 100)}%</p>
            ) : null}
