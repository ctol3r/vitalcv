...
/**
 * AcceptancePanel.tsx — Wave 99: Verifier Acceptance Flow
 *
 * Submits a presentation ID or JSON body for full verification,
 * then displays the acceptance report:
 * - credential summary per item
 * - issuer trust level
 * - per-check results
 * - final ACCEPTED / REJECTED / PENDING_REVIEW decision
 */
  ...
  return (
    <div className={`rounded-2xl border border-infra-border bg-white overflow-hidden shadow-sm ${className}`}>
      ...
      <div className="flex items-center gap-2 px-5 py-4 border-b border-infra-border bg-infra-surface">
        <FileSearch className="h-5 w-5 text-infra-blue" />
        <h2 className="font-bold text-foreground">Credential Acceptance</h2>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={loading || !presentationId.trim()}
              className="gap-1.5"
            >
              {loading
          ...
          <p className="text-xs text-muted-foreground">
            Clinicians can generate a presentation from their Holder Wallet.
          </p>
        </div>
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                  Credential Verification ({report.credentialResults.length})
                </h3>
