    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-infra-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-infra-muted ml-1">Wave 143</span>
        </div>
        <div className="flex items-center gap-2">
          {summary && (
            <span className="text-xs text-infra-muted">
              {summary.totalProviders} providers · exported {new Date(summary.exportedAt).toLocaleTimeString()}
            </span>
          )}
          <button
                ...
                {summary.entries.slice(0, 20).map((e) => (
                  <tr key={e.npi} className="border-b border-infra-border/50 hover:bg-infra-bg/50 transition-colors">
                    ...
                    <td className="px-3 py-2 text-center">
                      <span className={e.activeCredentials > 0 ? 'text-emerald-400 font-semibold' : 'text-infra-muted'}>{e.activeCredentials}</span>
                    </td>
