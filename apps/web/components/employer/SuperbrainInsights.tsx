    ...
    try {
      ...
      setResult({
        ...
        reasoningTrace: [
          ...
          `[ONTOLOGY] Resolved specialty: ${specialty}`,
          `[COMPLIANCE] Querying ${state} rules for specialty: ${specialty}`,
          `[COMPLIANCE] Typical credential types: [medical_license, board_certification]`,
          `[COMPLIANCE] Renewal window: 365 days`,
          `[VERIFICATION] Querying available artifacts for NPI: ${npi}`,
          `[VERIFICATION] Found artifacts from connected sources`,
          `[EVALUATION] ✓ medical_license — checked`,
          `[EVALUATION] ○ dea — not connected (institutional access required)`,
          `[EVALUATION] ○ board_certification — not connected (ABMS access required)`,
          `[RESULT] PARTIAL — Some requirements checked, others need institutional access for ${state}`,
        ],
      });
      void err;
    } finally {
