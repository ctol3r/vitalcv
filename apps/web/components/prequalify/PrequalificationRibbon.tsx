    ...
    <div className="w-full px-4 py-2.5 border-b border-white/8 bg-white/[0.02]">
      <div className="flex items-center gap-4 max-w-5xl mx-auto">
        ...
        <div className="flex-shrink-0 text-xs text-muted-foreground">
          Prequalification <span className="text-foreground font-medium">{pct}%</span>
        </div>
        ...
        <div className="flex items-center gap-2 flex-1 overflow-x-auto">
          {steps.map((step, i) => (
            <div key={step.key} className="flex items-center gap-1 flex-shrink-0">
              ...
              <span className={`text-xs ${step.done ? 'text-green-400' : 'text-muted-foreground'}`}>
                {step.label}
              </span>
              {i < steps.length - 1 && (
                <ChevronRight className="w-3 h-3 text-muted-foreground/30 flex-shrink-0 ml-1" />
              )}
            </div>
          ))}
        </div>
      ...
      {/* Progress bar */}
      <div className="mt-1.5 max-w-5xl mx-auto h-0.5 rounded-full bg-muted overflow-hidden">
        <div
