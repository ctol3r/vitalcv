...
function ListSection({
  ...
  return (
    <div className="space-y-2">
      <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/24">{title}</p>
    ...
    <Card className="gap-0 rounded-2xl border-white/8 bg-white/[0.03] py-0 shadow-none">
      ...
      <CardContent className="space-y-5 px-5 py-4">
        <div className="space-y-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/24">Major dimensions</p>
          <div className="space-y-2">
            {posture.dimensions.map((dimension) => {
              ...
              return (
                <div
                  key={dimension.id}
                  className="rounded-xl border border-white/6 bg-black/10 px-4 py-3"
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      ...
                      {checkedAt ? (
                        <p className="mt-2 text-[11px] text-white/24">Checked {checkedAt}</p>
                      ) : null}
                    </div>
        ...
        <div className="space-y-4 border-t border-white/6 pt-4">
          ...
          {safeItems.length === 0 ? (
            <div className="space-y-2">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/24">Safe to rely on now</p>
          ...
          {attentionCount > 0 ? (
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/24">Needs attention</p>
              <p className="text-xs leading-relaxed text-white/38">
