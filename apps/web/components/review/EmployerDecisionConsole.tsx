...
export function EmployerDecisionConsole({
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-0 font-sans">
      ...
      {/* ── 1. Decision Header ── */}
      <div className={`rounded-t-xl border-b-0 border-2 ${cfg.border} ${cfg.bg} px-6 py-6`}>
        {/* Decision verdict — answer to "Can I proceed?" — dominates the header */}
        ...
        <p className={`text-xl font-bold leading-snug ${cfg.color} mb-5`}>{nextAction}</p>

        {/* Clinician identity — context beneath the verdict */}
        <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-4 border-t ${cfg.border} border-opacity-40`}>
          <div>
            <h1 className="text-base font-semibold text-slate-700">{name}</h1>
...
function LaneRow({ lane }: { lane: LaneSnapshot }) {
  ...
  const isVerified = lane.status === 'verified';
  const isAdverse = lane.status === 'adverse';

  const containerClass = isAdverse
    ? 'border-2 border-red-300 rounded-lg overflow-hidden bg-red-50'
    : isVerified
    ? 'border border-slate-100 rounded-lg overflow-hidden opacity-60'
    : 'border border-slate-200 rounded-lg overflow-hidden';

  const nameLabelClass = isAdverse
    ...
    <div className={containerClass}>
      ...
      >
        <div className="flex items-center gap-3">
          {isVerified && <span className="text-green-500 text-xs">✓</span>}
          <span className={nameLabelClass}>
...
function ActionButton({
  ...
  const base = 'w-full text-left rounded-xl border transition-colors disabled:opacity-40';
  const styles = {
    primary:   `${base} px-4 py-4 bg-slate-900 hover:bg-slate-700 text-white border-slate-900`,
    secondary: `${base} px-3 py-2.5 bg-white hover:bg-slate-50 text-slate-700 border-slate-200`,
    ghost:     `${base} px-3 py-2 bg-transparent hover:bg-slate-100 text-slate-500 border-transparent`,
  }[variant];

  return (
    <button className={styles} disabled={disabled || loading} onClick={onClick}>
      <div className={variant === 'primary' ? 'font-bold text-base mb-0.5' : 'font-semibold text-sm mb-0.5'}>
        {loading ? 'Sending…' : label}
      </div>
      <div className="text-xs opacity-70 mb-1">{description}</div>
      <div className="text-[10px] opacity-50">{impact}</div>
    </button>
  );
}
