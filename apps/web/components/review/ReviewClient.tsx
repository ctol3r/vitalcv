...
export default function ReviewClient({ passport, contextId, bundleId, sharedBy }: Props) {
  ...
  async function handleReject() {
    if (!window.confirm('Are you sure? This decision will be recorded.')) return;
    await runEmployerAction({
      intent: 'reject',
      endpoint: 'reject',
      body: {
        ...
        reason: blocked.length > 0
          ? `Candidate rejected. Blockers: ${blocked.slice(0, 3).join(', ')}`
      ...
      <div className="w-full max-w-3xl space-y-6">
          ...
          <div className="rounded-2xl border border-white/8 bg-black/15 px-4 py-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] text-white/20 mt-0.5">Estimate only — based on current readiness score, not a guarantee</p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-2 mt-4 space-y-6">
            ...
            <div className="border-t border-white/10 pt-4 space-y-1 text-sm">
              ...
              <div className="pt-1">
                ...
                <p className="text-[10px] text-white/25 mt-0.5">Estimate only — based on current readiness score, not a guarantee</p>
              </div>
        ...
        {actionState.phase === 'idle' || actionState.phase === 'downloading' ? (
          <SectionReveal delay={0.1}>
            ...
            >
              {blocked.length > 0 ? `Proceed with Credentialing Head Start (PSV pending · ${blocked.length} blocker${blocked.length === 1 ? '' : 's'} noted)` : 'Proceed with Credentialing Head Start (PSV pending)'}
            </Button>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                {freshnessEntries.filter(e => e.stale || e.unchecked).length > 0
                  ? `Request Updated Data (${freshnessEntries.filter(e => e.stale || e.unchecked).length} stale · manual during pilot)`
                  : 'Request Updated Data (manual during pilot)'}
              </Button>
              <Button
            ...
            {/* Reject — visually de-emphasized, destructive */}
            <Button
              onClick={handleReject}
              disabled={!canPersistActions || actionState.phase === 'downloading'}
              variant="outline"
              className="min-h-[44px] w-full rounded-xl border-red-500/20 bg-transparent py-3 text-xs font-medium text-red-400/60 hover:border-red-500/40 hover:bg-red-500/8 hover:text-red-400/90"
            >
              Reject Candidate
            </Button>
