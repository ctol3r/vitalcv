...
export async function POST(
  ...
  try {
    const response = await fetch(`${MARKETPLACE_BACKEND}/api/applications/${encodeURIComponent(appId)}/start`, {
      method: 'POST',
    ...
    return NextResponse.json({ error: 'Actual-start confirmation is temporarily unavailable.' }, { status: 503, headers: PRIVATE_RESPONSE });
  }
}
