...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Forward x-clerk-user-id AND Authorization: Bearer so this survives the
  // backend's CLERK_JWT_VERIFICATION=enforce flip (additive/safe in off+shadow).
  const identity = await buildIdentityHeaders({ userId: session.userId });
