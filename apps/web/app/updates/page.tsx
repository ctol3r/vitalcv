...
export default async function UpdatesPage() {
  ...
  const envDot = deploy.env === 'production' ? 'bg-emerald-400 animate-pulse' : deploy.env === 'preview' ? 'bg-amber-400' : 'bg-white/30';
    ...
    <div className="min-h-screen bg-vt-surface-ops-base px-4 sm:px-6 py-12 sm:py-16 text-white">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-10 sm:mb-14">
          ...
          <p className="text-sm text-white/40">What shipped, what's building, what's next.</p>
        </div>

        {/* Deploy status panel */}
        <section className="mb-10 rounded-2xl border border-white/8 bg-white/3 p-5 sm:p-6">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">Deployment</p>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div>
              <p className="text-[10px] text-white/30 mb-1 uppercase tracking-wide">Environment</p>
            ...
            <div>
              <p className="text-[10px] text-white/30 mb-1 uppercase tracking-wide">Commit</p>
              <p className="font-mono text-sm text-white/80">{deploy.sha}</p>
            </div>
            <div>
              <p className="text-[10px] text-white/30 mb-1 uppercase tracking-wide">Branch</p>
              <p className="font-mono text-sm text-white/80 truncate">{deploy.branch}</p>
            </div>
            <div>
              <p className="text-[10px] text-white/30 mb-1 uppercase tracking-wide">Deployed</p>
          ...
          {deploy.message && (
            <p className="mt-4 pt-4 border-t border-white/6 text-xs text-white/40 font-mono truncate">
              {deploy.message}
            </p>
          )}
          {deploy.env === 'preview' && (
        ...
        <section className="mb-10">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">Currently In Progress</p>
          <div className="space-y-2">
            {IN_PROGRESS.map((item) => (
              <div key={item.label} className="flex items-start gap-3 rounded-xl border border-white/6 bg-white/2 px-4 py-3">
                <span className={`mt-0.5 text-[10px] font-bold uppercase tracking-wide min-w-[60px] ${STATUS_COLORS[item.status] ?? 'text-white/30'}`}>
                  {item.status}
                </span>
        ...
        {/* Changelog */}
        <section>
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">Recent Changes</p>
            ...
            {RECENT_CHANGES.map((entry) => (
              <div key={`${entry.date}-${entry.tag}`} className="rounded-2xl border border-white/6 bg-white/2 p-5">
                <div className="flex flex-wrap items-center gap-2 mb-4">
                  ...
                  <span className={`inline-flex items-center rounded border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${TAG_COLORS[entry.tag] ?? 'bg-white/8 text-white/40 border-white/10'}`}>
                    {entry.tag}
                  </span>
                </div>
        ...
        {/* Footer nav */}
        <div className="mt-12 pt-8 border-t border-white/8 flex flex-wrap gap-4 text-sm text-white/30">
