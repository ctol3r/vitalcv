...
export async function runTJCDailyJob(options: TJCDailyJobOptions = {}): Promise<TJCDailyJobResult> {
  ...
  for (const entry of privileges) {
    if (!entry.clinicianDid) {
      continue;
    }
    const key = `${entry.clinicianDid}:${entry.orgId ?? 'none'}`;
    if (!targets.has(key)) {
      targets.set(key, { clinicianDid: entry.clinicianDid, orgId: entry.orgId ?? null });
    }
  }
