...
export async function GET(request: NextRequest) {
  ...
  const npi = searchParams.get('npi') || searchParams.get('number');
  ...
  try {
    ...
    // Determine entity type code
    const entityTypeCode = (enumType === 'NPI-1' || basic.enumeration_type === '1') ? 1 : 2;

    // Transform to our format
    const record: NpiRecord & { entityTypeCode: number } = {
      npi: result.number,
