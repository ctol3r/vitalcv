...
export const features: Record<string, FeatureFlag> = {
  ...
  // Digital Credentials Features (DC-016)
  chromeDigitalCredentials: {
    id: 'chrome_digital_credentials',
    enabled: process.env.NEXT_PUBLIC_ENABLE_CHROME_DC === 'true' || false,
    name: 'Chrome Digital Credentials',
    description: 'Enable Chrome Digital Credentials API integration',
    betaOnly: true,
  },
  oid4vpVerifier: {
    id: 'oid4vp_verifier',
    enabled: process.env.NEXT_PUBLIC_ENABLE_OID4VP === 'true' || true,
    name: 'OID4VP Verifier',
    description: 'Enable OID4VP presentation request and verification flows',
    requiredRoles: ['verifier', 'admin'],
  },
  oid4vpWallet: {
    id: 'oid4vp_wallet',
    enabled: process.env.NEXT_PUBLIC_ENABLE_OID4VP === 'true' || true,
    name: 'OID4VP Wallet',
    description: 'Enable OID4VP presentation in wallet',
    requiredLevel: 1,
  },
  applyWithVitalCV: {
    id: 'apply_with_vitalcv',
    enabled: process.env.NEXT_PUBLIC_ENABLE_APPLY_VITALCV === 'true' || true,
    name: 'Apply with VitalCV',
    description: 'Enable "Apply with VitalCV" button on job postings',
