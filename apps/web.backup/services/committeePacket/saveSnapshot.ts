...
const STORAGE_ROOT = process.env.COMMITTEE_PACKET_STORAGE_ROOT ?? path.join(process.cwd(), 'artifacts', 'committee-packets');
const KEY_VERSION = process.env.COMMITTEE_PACKET_KEY_VERSION ?? 'v1';
const ENCRYPTION_KEY = process.env.COMMITTEE_PACKET_ENCRYPTION_KEY;
...
function getEncryptionKey(): Buffer {
  ...
  const buffer = ENCRYPTION_KEY.length === 43 || ENCRYPTION_KEY.length === 44
    ? Buffer.from(ENCRYPTION_KEY, 'base64')
    : Buffer.from(ENCRYPTION_KEY, 'hex');
...
function encryptPayload(payload: string): EncryptionResult {
  const key = getEncryptionKey();
  const iv = randomBytes(16);
  const cipher = createCipheriv('aes-256-gcm', key, iv);
...
function decryptPayload(data: EncryptionResult): string {
  const key = getEncryptionKey();
...
async function writeEncryptedSnapshot(clinicianId: string, packetId: string, payload: EncryptionResult, metadataHash: string) {
  const targetDir = path.join(STORAGE_ROOT, clinicianId);
  await fs.mkdir(targetDir, { recursive: true });
  const filePath = path.join(targetDir, `${packetId}.json.enc`);
  const content = JSON.stringify(
...
export async function saveCommitteePacketSnapshot(params: SaveSnapshotParams): Promise<CommitteePacketRecord> {
  ...
  const record = await prisma.committeePacket.create({
    data: {
      ...
      packetId,
      snapshotAt,
      status: status as CommitteePacketStatus,
      metadataHash,
      storagePath,
      snapshotSize,
      encryptionKeyVersion: KEY_VERSION,
