...
export async function generateCommitteeNarrative(
  ...
  if (options?.llmClient && !deterministic) {
    try {
      ...
      log.error(`LLM client ${options.llmClient.name} failed: ${(error as Error).message}`);
    }
  }
...
function buildOverview(
  ...
  return `${clinician.name}${clinician.specialty ? ` (${clinician.specialty})` : ''} • Packet ${
    cycle.packetId
  } • ${coverage} • Risk ${riskTier}. Highlights reference risk (${sections.risk.confidence}), quality (${sections.quality.confidence}), licensure (${sections.licensure.confidence}), and payer (${sections.payer.confidence}) context.`;
}
...
function buildPrompt(
  ...
  const lines: string[] = [
    'You are drafting a credentialing committee narrative. Use only the provided data. Do not invent facts.',
    '',
    `Clinician: ${packet.clinician.name} (${packet.clinician.specialty ?? 'Specialty n/a'})`,
    `Packet: ${packet.cycle.packetId}`,
  ];
