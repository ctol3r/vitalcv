...
const log = getServiceLogger('committeePacket/explainPayer');
...
export function explainPayerSection(
  ...
  if (!profile) {
    log.warn('Payer profile missing; returning empty section');
    return emptySection('payer');
  }
...
function buildPayerEvidence(profile: PayerProfile): EvidencePoint[] {
  ...
  const panelIssues =
    profile.panelIssues?.sort(
      (a, b) => compareBySeverity(a.severity, b.severity) || (b.openedAt ?? '').localeCompare(a.openedAt ?? '')
    ) ?? [];
