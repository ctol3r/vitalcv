...
export function getCurrentLifecycleStatus(
  ...
  if (!history.length) return null;
  const sorted = sortLifecycleHistory(history);
...
export function appendLifecycleEvent(
  ...
  const normalized = sortLifecycleHistory(history);
  const existingIndex = normalized.findIndex(
    (entry) => entry.status === event.status && equalsDate(entry.occurredAt, event.occurredAt)
  );
...
function equalsDate(a: unknown, b: unknown): boolean {
