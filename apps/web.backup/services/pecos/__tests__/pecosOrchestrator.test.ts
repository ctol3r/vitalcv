...
describe('PECOS orchestrator', () => {
  beforeEach(() => {
    ...
    mockPrisma.pECOSProvider.upsert.mockResolvedValue({
      id: 'provider-1',
    });
  });

  it('processes enrollments and updates PECOS records', async () => {
  ...
  it('records failures when lookup errors persist', async () => {
    ...
    expect(result.failures[0].reason).toContain('API down');
