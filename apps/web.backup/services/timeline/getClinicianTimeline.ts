import {
  ...
  Prisma,
  PrismaClient,
  TimelineEventSeverity as PrismaTimelineEventSeverity,
  User,
...
const CREDENTIAL_EVENT_MAPPING: Record<
  CredentialTimelineEventType,
  { phase: TimelinePhase; title: string }
> = {
  VC_ISSUED: { phase: 'identity', title: 'Verifiable credential issued' },
  LICENSE_VERIFIED: { phase: 'licensure', title: 'License verification completed' },
  BOARD_VERIFIED: { phase: 'board', title: 'Board certification verified' },
  DEA_VERIFIED: { phase: 'dea', title: 'DEA registration verified' },
  PECOS_VERIFIED: { phase: 'enrollment', title: 'PECOS enrollment updated' },
  NPDB_QUERY: { phase: 'quality', title: 'NPDB query executed' },
  PRIVILEGE_GRANTED: { phase: 'privileging', title: 'Privilege granted' },
  FPPE_STARTED: { phase: 'quality', title: 'FPPE review started' },
  FPPE_COMPLETED: { phase: 'quality', title: 'FPPE review completed' },
  OPPE_CYCLE: { phase: 'quality', title: 'OPPE cycle recorded' },
  REVALIDATION_CREATED: { phase: 'quality', title: 'Revalidation task created' },
  COMPACT_ELIGIBILITY_UPDATED: { phase: 'licensure', title: 'Compact eligibility updated' },
  DISCREPANCY_FOUND: { phase: 'quality', title: 'Discrepancy detected' },
  DIRECTORY_NORMALIZED: { phase: 'identity', title: 'Directory record normalized' },
};
...
function mapSeverityToStatus(severity: PrismaTimelineEventSeverity | null): TimelineEventStatus {
  switch (severity) {
    case 'CRITICAL':
      return 'failed';
    case 'WARNING':
...
export async function buildClinicianTimeline(options: TimelineBuildOptions): Promise<TimelineBuildResult> {
  ...
  const [
    ...
    fetchLoggedCredentialEvents(clinician),
  ]);
...
async function fetchQualityEvents(clinician: ClinicianWithUser): Promise<TimelineEvent[]> {
  ...
  const [fppeCases, oppeCases, revalidationTasks, npdbMonitors] = await Promise.all([
...
async function fetchLoggedCredentialEvents(clinician: ClinicianWithUser): Promise<TimelineEvent[]> {
  const entries = await prisma.credentialTimelineEvent.findMany({
    where: { clinicianId: clinician.id },
    orderBy: { timestamp: 'desc' },
  });

  return entries.map((entry) => {
    const mapping = CREDENTIAL_EVENT_MAPPING[entry.eventType] ?? {
      phase: 'quality' as TimelinePhase,
    ...
    return {
      ...
      status: mapSeverityToStatus(entry.severity ?? null),
      source: 'credential_timeline',
