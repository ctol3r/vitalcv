...
function buildSnapshot(overrides?: DeepPartial<TJCSignalSnapshot>): TJCSignalSnapshot {
  ...
  applyOverrides(clone, overrides as Record<string, unknown>);
  return clone;
}
...
function applyOverrides(target: Record<string, unknown>, source: Record<string, unknown>) {
  for (const [key, value] of Object.entries(source)) {
    if (value && typeof value === 'object' && !Array.isArray(value)) {
      ...
      applyOverrides(target[key] as Record<string, unknown>, value as Record<string, unknown>);
      continue;
    }
    target[key] = value;
  }
...
describe('TJC rule evaluation', () => {
  ...
  it('warns when FPPE completion data is missing', () => {
    const rules = evaluateTjcRules(
      buildSnapshot({
        fppe: { completedCount: 0, openCount: 1, overdueCount: 0 },
      })
    );
  ...
  it('warns MS03 when OPPE metrics show issues', () => {
    const rules = evaluateTjcRules(
      buildSnapshot({
        oppe: { metrics: { complications: 1, issuesFlagged: 1 } },
      })
    );
  ...
  it('fails MS05 when a license is expired', () => {
    const rules = evaluateTjcRules(
      buildSnapshot({
        licenses: { expired: 1, active: 1 },
      })
    );
    expect(getStandardStatus(rules, 'MS05')).toBe('FAIL');
  });

  it('fails MS06 when PSV is stale', () => {
    const staleIso = new Date(now.getTime() - 130 * 24 * 60 * 60 * 1000).toISOString();
    const rules = evaluateTjcRules(
      buildSnapshot({
        psv: { checkedAt: staleIso, isFresh: false, freshUntil: staleIso, daysRemaining: -10 },
      })
    );
  ...
  it('fails MS06 when NPDB adverse action is present', () => {
    const rules = evaluateTjcRules(
      buildSnapshot({
        npdb: { status: 'ADVERSE_ACTION', adverseActionCount: 1 },
      })
    );
    expect(getStandardStatus(rules, 'MS06')).toBe('FAIL');
  });
  ...
  it('warns MS08 when competency actions remain open', () => {
    const rules = evaluateTjcRules(
      buildSnapshot({
        competency: { outstandingActions: 1 },
      })
    );
