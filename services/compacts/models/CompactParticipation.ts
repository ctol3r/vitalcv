...
export const COMPACT_TYPES = ['IMLC', 'NLC', 'PSYPACT', 'APRN', 'PT', 'COUNSELING'] as const;
...
export function buildParticipationIndex(records: CompactParticipationRecord[]): CompactParticipationIndex {
  ...
  for (const record of records) {
    const bucket = index[record.compactType] ?? {};
    bucket[record.state] = record;
    index[record.compactType] = bucket;
  }
  return index;
}
...
export function summarizeParticipation(records: CompactParticipationRecord[]) {
  ...
  for (const record of records) {
    ...
    const counts = summary.get(record.compactType)!;
    if (record.status === 'ACTIVE') counts.active += 1;
    if (record.status === 'SUSPENDED') counts.suspended += 1;
    if (record.status === 'ORGANIZATIONAL_ONLY') counts.organizational += 1;
  }
  return summary;
}
