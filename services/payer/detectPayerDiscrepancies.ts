...
function detectMissingCaqhData(context: DiscrepancyContext): DiscrepancyFinding[] {
  ...
  if (profile && profile.missingSections?.length) {
    findings.push({
      code: 'MISSING_CAQH_DATA',
      severity: 'warning',
...
function detectSpecialtyMismatch(context: DiscrepancyContext): DiscrepancyFinding[] {
  ...
  if (!payerSpecialties.length || !context.clinicianSpecialty) {
    return [];
  }

  const matches = payerSpecialties.some(
    (specialty) => specialty.toLowerCase() === context.clinicianSpecialty!.toLowerCase()
  );
  ...
  return [
    {
      ...
      recommendation: 'Select a supported specialty or choose a different payer',
    ...
    return [
      {
        ...
        recommendation: 'Re-attempt when panel reopens or request an exception with payer relations',
      },
    ];
  }
    ...
    return [
      {
        ...
        recommendation: 'Review rejection feedback and escalate as needed',
      },
    ];
  }
