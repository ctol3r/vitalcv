...
import {
  ...
  REGION_DB_ENDPOINTS,
  REGION_ENDPOINTS,
  REGION_REGULATORY_FRAMEWORK,
  RegulatoryFramework,
} from './region-types';
// Re-export region types for backward compatibility
export {
  Region,
  REGION_DB_ENDPOINTS,
  REGION_ENDPOINTS,
...
export const TenantRegionCreateSchema = TenantRegionSchema.omit({
  ...
  allowedRegions: z.array(z.nativeEnum(Region)).min(1).optional(), // Will default to [homeRegion]
});
...
export function validateTenantRegion(data: unknown): TenantRegion {
  ...
  if (!parsed.allowedRegions.includes(parsed.homeRegion)) {
    throw new Error(
      `Home region '${
        parsed.homeRegion
      }' must be included in allowed regions: [${parsed.allowedRegions.join(', ')}]`,
    );
  }
...
export function validateTenantRegionCreate(data: unknown): TenantRegion {
  const input = TenantRegionCreateSchema.parse(data);
  ...
  // Ensure homeRegion is in allowedRegions
  if (!allowedRegions.includes(input.homeRegion)) {
    allowedRegions.push(input.homeRegion);
  }

  const tenant: TenantRegion = {
    ...input,
    allowedRegions,
    regulatoryFramework: REGION_REGULATORY_FRAMEWORK[input.homeRegion],
    createdAt: new Date(),
    updatedAt: new Date(),
  };

  return validateTenantRegion(tenant);
}
