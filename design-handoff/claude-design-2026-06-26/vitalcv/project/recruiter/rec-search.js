/* ══════════════════════════════════════════════════════════════════════════
   rec-search.js — ⌘K unified search across the recruiter network
   Clinicians · Opportunities. Mirrors the W300 ecosystem search affordance.
   ══════════════════════════════════════════════════════════════════════════ */
(function () {
  ...
  function search(q, scope) {
    const idx = window.REC.searchIndex;
    ...
    return pool.map(r => {
      ...
      let s = 0;
      if (r.title.toLowerCase().startsWith(t)) s += 5;
      if (r.title.toLowerCase().includes(t)) s += 3;
  ...
  function counts() {
    const idx = window.REC.searchIndex, c = {};
  ...
  function render() {
    ...
    const input = el('input', { type: 'text', placeholder: 'Search clinicians, specialties, requisitions…', value: state.q, spellcheck: 'false' });
    input.addEventListener('input', () => { state.q = input.value; state.cursor = 0; render(); input.focus(); });

    const scopes = el('div', { class: 'searchscopes' },
      SCOPES.map(s => el('button', { class: 'scope' + (state.scope === s.id ? ' on' : ''),
    ...
    if (!state.results.length) {
      list.appendChild(el('div', { class: 'sempty' }, state.q ? `No matches for “${state.q}”.` : 'Type to search the network.'));
    } else {
      const order = ['clinician', 'opportunity'];
      let flatIdx = 0;
      const groups = state.scope === 'all' ? order.filter(t => state.results.some(r => r.type === t)) : [state.scope];
      groups.forEach(t => {
        ...
        rows.forEach(r => {
          ...
          list.appendChild(el('div', {
            ...
            onclick: () => go(r), onmouseenter: () => { state.cursor = myIdx; updateCursor(); },
          },
            el('div', { class: 'sic' }, icon(typeMeta(r.type).icon, 16)),
            el('div', { class: 'stx' }, el('div', { class: 'a' }, r.title), el('div', { class: 'b' }, r.sub)),
    ...
    const panel = el('div', { class: 'searchpanel', onclick: (e) => e.stopPropagation() },
      el('div', { class: 'shead' }, icon('search', 19), input, el('span', { class: 'esc' }, 'ESC')),
      scopes, list,
      el('div', { class: 'sfoot' },
        el('span', null, el('b', null, '↑↓'), ' navigate'),
        el('span', null, el('b', null, '↵'), ' open'),
        el('span', null, el('b', null, String(state.results.length)), ' results'),
        el('span', { style: { marginLeft: 'auto' } }, 'network index · clinicians + reqs')));

    root.appendChild(el('div', { class: 'scrim', onclick: close }, panel));
    setTimeout(() => input.focus(), 10);
  }

  function updateCursor() { document.querySelectorAll('.sresult').forEach((r, i) => r.classList.toggle('cursor', i === state.cursor)); }
  ...
  function scrollCursor() {
    const c = document.querySelector('.sresult.cursor'), box = document.querySelector('.sresults');
    if (!c || !box) return;
  ...
  function open() { state = { q: '', scope: 'all', cursor: 0, results: [] }; render(); document.addEventListener('keydown', onKey); }
  function close() { document.getElementById('searchRoot').innerHTML = ''; document.removeEventListener('keydown', onKey); }
  window.openRecSearch = open;
})();
