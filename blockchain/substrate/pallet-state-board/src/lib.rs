...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        BoardSynced(Vec<u8>),
        /// Revenue event anchored.
        RevenueRecorded(BillingEvent),
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        #[pallet::call_index(1)]
        #[pallet::weight(10_000)]
        pub fn submit_billing_event(origin: OriginFor<T>, event_id: Vec<u8>, units: u32) -> DispatchResult {
            let _ = ensure_signed(origin)?;
            let event = BillingEvent { event_id, units };
            Self::deposit_event(Event::RevenueRecorded(event));
            Ok(())
        }
    }
