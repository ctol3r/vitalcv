...
const PROHIBITED_CLAIMS: ReadonlyArray<{ phrase: string; fix: string }> = [
  ...
  // ---- CLAUDE.md banned strings not covered above (M1-8 completeness) ----
  { phrase: 'final verification without review', fix: 'Every decision-grade result requires policy review — remove.' },
  { phrase: 'source confirmed before response', fix: 'Sources are checked asynchronously — do not claim pre-response confirmation.' },
  { phrase: 'certified compliant', fix: 'Not certified — say "compliance-aligned by design".' },
];
...
// M1-8: an allowlist for legitimate technical-doc usage. Each entry is
// { file, phrase } — an exact repo-relative path + banned phrase that is
// intentionally present (e.g. a status page enumerating what VitalCV does NOT
// claim). Keep this list SMALL and reviewed; it is the escape hatch, not a mute.
interface AllowEntry { file: string; phrase: string; note: string }
let ALLOWLIST: AllowEntry[] = [];
try {
  const raw = readFileSync(join(repoRoot, 'scripts', 'copy-audit-allowlist.json'), 'utf8');
...
// Bare-"Verified" status-label detectors (case-sensitive, capital V, exact word).
// JSX text node: >Verified< (optionally with surrounding whitespace).
const BARE_VERIFIED_JSX = />\s*Verified\s*</;
// Standalone quoted value: 'Verified' / "Verified" / `Verified` used as a
// status/label value. Requires the quotes to hug the exact word.
...
function scanFile(absPath: string): void {
  ...
  if (dot < 0 || !SCANNABLE_EXT.has(absPath.slice(dot))) return;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const claim of NORMALIZED_CLAIMS) {
      if (haystack.includes(claim.needle) && !isAllowed(rel, claim.phrase)) {
        hits.push({
          file: rel,
          line: i + 1,
          phrase: claim.phrase,
    ...
    // CLAUDE.md: "No status label may be the bare word `Verified`." Narrow match —
    // a JSX text node (>Verified<) or a standalone quoted status value ('Verified').
    if ((BARE_VERIFIED_JSX.test(lines[i]) || BARE_VERIFIED_LITERAL.test(lines[i]))
        && !isAllowed(rel, 'Verified')) {
      hits.push({
        file: rel,
        line: i + 1,
        phrase: 'Verified (bare status label)',
        fix: 'No status label may be the bare word "Verified" — use "Checked", "Source-checked", or a coverage state.',
        text: lines[i].trim().slice(0, 160),
      });
    }
  }
