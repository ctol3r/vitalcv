...
/* -------------------------------------------------------------- the faces */
const F = {};
F.BLANK = (w) => `<div class="evr" style="--w:${w}">${spine}${head({tag:'Awaiting your number'})}${band(S.closed)}
...
F.WRITING = (w) => `<div class="evr" style="--w:${w}">${spine}${head({tag:'Reading your number', npi:'NPI 13•• ••• ••••'})}${band(S.closed)}
  <div class="evr-body"><ul class="evr-rows">${LANES.map(l=>`<li class="evr-row"><span class="evr-claim">${l}</span>
  <span class="stamp s-pend"><span class="g">○</span>Not checked</span></li>`).join('')}</ul></div></div>`;

F.RESOLVING = (w) => `<div class="evr" style="--w:${w}">${spine}${head({tag:'Querying sources', npi:NPI})}${band(S.opening)}
...
F.RETURNED = (w, scale) => `<div class="evr"${scale?` data-scale="${scale}"`:''} style="--w:${w}">${spine}${head({tag:ILLUS, name:HOLDER, npi:NPI})}${band(S.returned)}
  <div class="evr-body"><ul class="evr-rows">${ROWS.map(r=>`<li class="evr-row"><span>
  <span class="evr-claim">${r.c}</span><span class="evr-ret">${r.r}</span><span class="evr-prov">${r.p}</span></span>${stamp(r)}</li>`).join('')}</ul>
  <div class="evr-receipt">rcpt:nppes:8f2a…c41 · ES256 · P-256 · /.well-known/jwks.json</div></div></div>`;

F.INSPECTED = (w) => `<div class="evr" style="--w:${w}">${spine}${head({tag:ILLUS, name:HOLDER, npi:NPI})}${band(S.returned)}
  <div class="evr-body"><ul class="evr-rows">
  <li class="evr-row"><span><span class="evr-claim">Identity</span><span class="evr-ret">Located in the NPPES registry</span></span>${stamp(ROWS[0])}</li>
  ...
  ${ROWS.slice(1,3).map(r=>`<li class="evr-row"><span><span class="evr-claim">${r.c}</span><span class="evr-ret">${r.r}</span></span>${stamp(r)}</li>`).join('')}
  </ul></div></div>`;

F.DECIDING = (w) => `<div class="evr" style="--w:${w}">${spine}${head({tag:'Illustrative workflow', name:HOLDER, npi:NPI})}${band(S.returned)}
  <div class="evr-body"><ul class="evr-rows">${LEDGER.map(e=>`<li class="evr-row" data-travel="${e.t?'travels':'held'}"><span>
...
F.TRAVELLING = (w) => `<div style="display:flex;gap:calc(${w} * 0.07);align-items:flex-start">
  <div class="evr" style="--w:${w}">${spine}${head({tag:'Illustrative workflow', name:HOLDER, npi:NPI})}${band(S.returned)}
   <div class="evr-body"><ul class="evr-rows">${LEDGER.map(e=>`<li class="evr-row" data-travel="${e.t?'travels':'held'}"><span>
  ...
  <div class="evr evr--recipient" style="--w:calc(${w} * 0.62)">${spine}${head({tag:'Recipient frame'})}${band(['returned','returned','limited','closed','closed','closed'])}
...
F.SEALED = (w) => `<div class="evr evr--sealed" style="--w:${w}">${spine}${head({tag:ILLUS, npi:NPI})}${band(S.returned)}
...
const spineTest = [4,5,6].map(px=>`<div><p class="cap">Spine ${px}px</p>
  <div style="--spine:${px}px">${F.RETURNED('420px')}</div>
  <div class="covered thumb" style="--spine:${px}px;margin-top:14px"><div class="scaler">${F.RETURNED('420px')}</div></div></div>`).join('');
...
const apTest = [['closed','Nothing checked'],['opening','Request in flight'],['returned','Source answered'],['limited','Access or scope limitation'],['unavailable','System could not answer']]
  .map(([s,label])=>`<div><p class="cap">${s}</p>
   <div class="evr" style="--w:260px">${spine}${head({tag:label})}${band(Array(6).fill(s))}</div></div>`).join('');
...
<div class="grid"><div>${F.RETURNED('420px')}</div><div>${F.SEALED('420px')}</div></div>
`));
...
<div class="stage" style="width:1440px;height:900px;display:flex;align-items:center;padding-left:80px">
  <div style="margin-bottom:-120px">${F.RETURNED('1008px')}</div>
</div>
...
<div class="stage" style="width:1728px;height:1117px;display:flex;align-items:center;padding-left:110px;transform:scale(0.86);transform-origin:top left">
  <div>${F.RETURNED('1037px','hero')}</div>
</div>
`));
...
${order.map(f=>`<p class="cap" style="margin-top:26px">${f}</p><div class="grid">${F[f]('420px')}</div>`).join('')}

<p class="cap" style="margin-top:36px">Recognition test · 120px · all copy covered</p>
<div class="grid">${['BLANK','RESOLVING','RETURNED','INSPECTED','DECIDING','SEALED'].map(f=>`<div class="covered thumb"><p class="cap">${f}</p><div class="scaler">${F[f]('420px')}</div></div>`).join('')}</div>
`));
