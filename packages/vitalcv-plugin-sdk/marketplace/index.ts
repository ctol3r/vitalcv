...
export async function validateManifest(
	...
	if (!signedManifest.name) errors.push('Missing name');
	if (!signedManifest.version) errors.push('Missing version');
	if (!signedManifest.author) errors.push('Missing author');
	if (!signedManifest.entrypoint) errors.push('Missing entrypoint');
	if (!signedManifest.capabilities || signedManifest.capabilities.length === 0) {
		errors.push('Missing capabilities');
	}
	if (!signedManifest.signature) errors.push('Missing signature');
	if (!signedManifest.signedAt) errors.push('Missing signedAt');
 ...
 */
export async function testModulePackage(
 ...
 */
export async function createMarketplaceModule(
	config: MarketplaceModuleConfig,
	privateKey: string,
	files: string[]
): Promise<PackageManifest> {
